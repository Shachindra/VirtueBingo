...
func IntArrayToStringArray(intArray [3][9]int) [][]string {
	stringArray := make([][]string, len(intArray))
	for i, row := range intArray {
		stringArray[i] = make([]string, len(row))
		for j, num := range row {
			stringArray[i][j] = strconv.Itoa(num)
		}
	}
	return stringArray
}

func ReplaceZeroWithEmpty(input [][]string) [][]string {
	for i := range input {
		for j := range input[i] {
			if input[i][j] == "0" {
				input[i][j] = ""
